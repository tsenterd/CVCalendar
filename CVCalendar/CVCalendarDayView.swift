...
extension CVCalendarDayView {
    public func labelSetup() {
        ...
        totalLabel = UILabel()
        totalLabel.frame = CGRect(x: 0, y: -15, width: 100, height: 50)
        
        // if let ttl = total{
        //dayLabel!.text = String(Int(ttl))
        //   totalLabel.text = String(Int(ttl))
        //}
        totalLabel.text=String(date.day)
        totalLabel.textAlignment = NSTextAlignment.Left
        totalLabel.font = UIFont(name: "Avenir-Heavy", size: 8)
    ...
    public func setupDotMarker() {
        ...
        if let delegate = calendarView.delegate {
            if let shouldShow = delegate.dotMarker?(shouldShowOnDayView: self) where shouldShow {
                ...
                var (width, height): (CGFloat, CGFloat) = (15, 15)
                ...
                for (index, color) in (colors!).enumerate() {
                    ...
                    case 1:
                        x = frame.width / 2
                    case 2:
                        let inx = Double(index);
                        if index == 0{
                            x = frame.width * CGFloat(2.15+inx)/5.00 // frame.width * (2/5, 3/5)
                        }
                        else {
                            x = frame.width * CGFloat(2+inx)/5.00 // frame.width * (2/5, 3/5)
                        }
                    case 3:
                        let inx = Double(index)
                    ...
                    case 4:
                        let inx = Double(index)
                        x = frame.width * CGFloat(2+index)/7.00 // frame.width * (1/3, 1/2, 2/3)
                    default:
                        break
                    }
